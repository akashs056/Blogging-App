...
public class homeAdapter extends  RecyclerView.Adapter<homeAdapter.viewHolder> {
    ...
    public void onBindViewHolder(@NonNull viewHolder holder, int position) {
        ...
        holder.binding.save.setImageResource(R.drawable.non_save);
                                    ...
                                    FirebaseDatabase.getInstance().getReference().child("posts")
                                            ...
                                            .removeValue().addOnSuccessListener(new OnSuccessListener<Void>() {
                                                ...
                                                public void onSuccess(Void unused) {
                                                    FirebaseDatabase.getInstance().getReference().child("posts")
                                                            .child(post.getPostId()).child("postLike").setValue(post.getPostLike()-1);
                                                }
                                            });
                                }
                            });
        ...
        FirebaseDatabase.getInstance().getReference().child("Users").child(FirebaseAuth.getInstance().getUid())
                .child("savedPost").child(post.getPostId()).addListenerForSingleValueEvent(new ValueEventListener() {
                    @Override
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        if (snapshot.exists()){
                            holder.binding.save.setImageResource(R.drawable.full_save);
                            holder.binding.save.setOnClickListener(new View.OnClickListener() {
                                @Override
                                public void onClick(View v) {
                                    if (post.getPostId() != null) {
                                        FirebaseDatabase.getInstance().getReference().child("Users").child(FirebaseAuth.getInstance().getUid())
                                                .child("savedPost").child(post.getPostId()).removeValue().addOnSuccessListener(new OnSuccessListener<Void>() {
                                                    @Override
                                                    public void onSuccess(Void unused) {
                                ...
                                public void onClick(View v) {
                                    if (post.getPostId() != null) {
                                        FirebaseDatabase.getInstance().getReference().child("Users")
                                                ...
                                                .child(post.getPostId()).setValue(true).addOnSuccessListener(new OnSuccessListener<Void>() {
                                                    @Override
                                                    public void onSuccess(Void unused) {
                                                        holder.binding.save.setImageResource(R.drawable.full_save);
                                                    }
    ...
    public class viewHolder extends RecyclerView.ViewHolder{
        SampleRvBinding binding;
        public viewHolder(@NonNull View itemView) {
            super(itemView);
            binding=SampleRvBinding.bind(itemView);
        }
    }
}
